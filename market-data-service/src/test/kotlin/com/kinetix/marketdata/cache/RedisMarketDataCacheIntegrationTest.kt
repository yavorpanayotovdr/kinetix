...
class RedisMarketDataCacheIntegrationTest : FunSpec({

    val connection = RedisTestSetup.start()
    val cache: MarketDataCache = RedisMarketDataCache(connection)

    beforeEach {
        connection.sync().flushall()
    }

    test("put and get market data point") {
        val p = point()
        cache.put(p)
    ...
    test("preserves all market data sources") {
