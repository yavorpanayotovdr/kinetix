...
**Date:** 2026-03-01
**Reviewed by:** Quant, UX Designer, Data Analyst, Compliance Officer, Trader, Architect, SRE
...
Seven specialist agents conducted an independent, code-level review of the entire Kinetix platform. The system has **strong fundamentals**: clean service decomposition, comprehensive testing, solid event-driven architecture, and a rich risk analytics engine. However, significant gaps exist across **derivative pricing** (no options models), **trade lifecycle** (no amend/cancel), **regulatory governance** (non-tamperproof audit trail), **production infrastructure** (no PDBs, no network policies, plaintext secrets), and **data operations** (no DLQ, no retention policies).
...
| Infrastructure | 3/5 | Excellent CI/CD, full observability stack | No PDB, no secrets management, no DB backups |
...
### 1.7 Add PodDisruptionBudgets
**Source:** SRE
**Problem:** No PDBs anywhere. With SPOT instances, all replicas of a service could be evicted simultaneously.
**Action:** Add `minAvailable: 1` PDB for all services with 3+ replicas in prod.
**Effort:** Small

### 1.8 Kubernetes Security Contexts
**Source:** SRE
**Problem:** Pod specs don't enforce `runAsNonRoot`, `readOnlyRootFilesystem`, or capability drops.
**Action:** Add security contexts to all Helm chart templates:
```yaml
securityContext:
  runAsNonRoot: true
...
**Source:** SRE
**Problem:** Plaintext passwords in Helm values files. Redis has no authentication.
**Action:** Integrate `external-secrets-operator` with AWS Secrets Manager. Enable Redis auth in production.
**Effort:** Medium
...
## Phase 4: Data & Infrastructure Hardening (Weeks 6-9)
...
### 4.5 Database Backup Strategy
**Source:** SRE
**Problem:** No backup configuration for PostgreSQL. No RPO/RTO targets.
**Action:** Either migrate to AWS RDS with automated backups, or implement pgBackRest with WAL archiving to S3. Target RPO < 1 hour, RTO < 4 hours.
**Effort:** Medium

### 4.6 Network Policies
**Source:** SRE
**Problem:** All pods can communicate with all other pods. No network segmentation.
**Action:** Define ingress/egress rules per service. UI -> gateway only, gateway -> backend services, risk-orchestrator -> risk-engine, services -> PostgreSQL/Kafka/Redis.
**Effort:** Medium

### 4.7 TLS on Ingress
**Source:** SRE
**Problem:** Ingress has no TLS. Public-facing without HTTPS.
**Action:** Add ACM certificate ARN annotation, configure HTTPS listener, redirect HTTP to HTTPS.
**Effort:** Small

### 4.8 Enable OTel Traces and Metrics in Helm
**Source:** SRE
**Problem:** `OTEL_METRICS_EXPORTER` and `OTEL_TRACES_EXPORTER` set to `"none"` in Helm values. Observability is partially disabled in K8s.
**Action:** Change to `"otlp"` across all service Helm values.
**Effort:** Small

### 4.9 Kafka Replication Factor
**Source:** SRE
**Problem:** All topics have `replicationFactor: 1` even with 3 brokers in prod. Single broker failure causes data loss.
**Action:** Set replication factor to 3 for all prod topics.
**Effort:** Small

### 4.10 Provision All Kafka Topics Explicitly
**Source:** Data Analyst
...
### 4.12 Add Missing Database Indexes
**Source:** Data Analyst
...
**Source:** UX Designer
**Problem:** No mobile/tablet breakpoints. Fixed widths break on small screens.
**Action:** Stack VaR dashboard vertically on small screens. Make position table horizontally scrollable with sticky first column. Make WhatIfPanel full-screen on mobile.
**Effort:** Medium

### 5.4 Position Grid Pagination/Virtualisation
**Source:** UX Designer
...
### 5.6 Consolidate Currency Formatting
**Source:** UX Designer
...
**Source:** SRE
**Problem:** Zero runbooks, playbooks, or on-call documentation.
**Action:** Create runbooks for: incident response, database failover, Kafka rebalancing, service rollback, on-call escalation.
**Effort:** Medium

### 6.13 Infrastructure Alert Rules
**Source:** SRE
**Problem:** Alerts only cover business domain. Missing: pod restarts, 5xx rates, memory, connection pool exhaustion.
**Action:** Add Prometheus alert rules for infrastructure-level failures.
**Effort:** Small

### 6.14 Workspace Customisation
**Source:** UX Designer
...
|----------|-------|-----------|
| **Critical / Phase 1** | 9 | Correctness fixes, security, audit integrity, decoupling |
...
| **Medium / Phase 4** | 12 | Data ops, infra hardening, retention, networking |
| **Medium / Phase 5** | 9 | Accessibility, responsiveness, dark mode, UX polish |
| **Lower / Phase 6** | 14 | Advanced models, governance, margin, customisation |
| **Total** | **55** | |
...
*This plan was generated from independent reviews by 7 specialist agents. Each agent read actual source code and formed opinions based on what exists in the codebase, not assumptions.*
